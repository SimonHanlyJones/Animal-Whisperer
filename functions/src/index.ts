...
exports.animalChat = onCall(
  { secrets: ["OPENAI", "APPCHECK"] },
  async (request) => {
    // Authentication / user information is automatically added to the request.
    //   if (request.auth) {
    //     const uid = request.auth.uid || null;
    ...
    if (!request.data.timestamp || !request.data.signature) {
      throw new HttpsError(
        "failed-precondition",
        "The Animal Chat function must be called with a timestamp and signature argument"
      );
    }
    ...
    const backSignature = createSignature(
      request.data.messages,
      request.data.timestamp
    );
    if (backSignature !== request.data.signature) {
      throw new HttpsError(
        "failed-precondition",
        "The signature in the request is invalid."
      );
    }
